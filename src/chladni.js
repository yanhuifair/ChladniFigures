...
// 圆形位移场：ψ = J_n(z·r)·cos(nθ)，z 取 J_n' 零点 → 满足自由边「径向斜率=0」
// 边界条件（自由圆板的常用近似；严格薄板自由圆板本征函数应含修正贝塞尔 I_n 项，
// 本实现取纯 J_n，节线图样正确且代价低）。
...
//   · 板越大(L↑) → 同模式频率越低（∝ 1/L²）
//   · 板越硬(s↑) → 同模式频率越高（∝ √s）
// f₀=55Hz 为参考基准（L₀=40cm, s₀=1）时的 (m²+n²)=1 基频；
// 注意 (1,2) 模式实际频率为 f₀·(1+4)=275Hz，55Hz 只是量纲基准值。
